...
#include "tensorflow/core/example/example.pb_text-impl.h"
...
void AppendProtoDebugString(
    ...
    const ::tensorflow::Example& msg) {
  if (msg.has_features()) {
    o->OpenNestedMessage("features");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.features());
  ...
  while(true) {
    ...
    if (identifier == "features") {
      ...
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_features())) return false;
    }
  }
}
...
void AppendProtoDebugString(
    ...
    const ::tensorflow::SequenceExample& msg) {
  if (msg.has_context()) {
    o->OpenNestedMessage("context");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.context());
    o->CloseNestedMessage();
  }
  if (msg.has_feature_lists()) {
    o->OpenNestedMessage("feature_lists");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.feature_lists());
    o->CloseNestedMessage();
  }
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "context") {
      ...
      ProtoSpaceAndComments(scanner);
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_context())) return false;
    }
    else if (identifier == "feature_lists") {
      ...
      ProtoSpaceAndComments(scanner);
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_feature_lists())) return false;
    }
  }
}
