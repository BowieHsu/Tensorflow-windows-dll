 ...
 public:
  ...
  void CombineSegments(const T* segments,
    ...
    for (const auto& kv : groups) {
	  ...
	  //将sin cos求和改为角度求和
	  //float theta_vector = 0.0f;
	  //for (int i = 0; i < segments.size(); i++) {
		 // const float theta = std::atan(segments[i][5] / segments[i][4]);
		 // theta_vector += theta;
	  //}

	  //const T combined_theta = theta_vector / segments.size();
	  //const T m = std::tan(combined_theta);

	  //已知所有seg，求seg的凸包
	  //std::vector<float>

      // find mx + b
	  const T combined_theta_cos = std::accumulate(
		  segments.begin(), segments.end(), static_cast<T>(0),
		  [](T x, const seg_t& seg) { return x + seg[4]; }
	  ) / static_cast<T>(segments.size());
	  const T combined_theta_sin = std::accumulate(
		  segments.begin(), segments.end(), static_cast<T>(0),
      ...
      for (const seg_t& seg : segments) {
        ...
        proj_points.push_back({proj_x, proj_y});
      }

      // find end points by finding the min and max of 
      // the inner products of (cx, cy) and (1, m)
      std::vector<T> inner_products(segments.size());
      for (int i = 0; i < (int)segments.size(); ++i) {
        T cx = segments[i][0];
        T cy = segments[i][1];
        inner_products[i] = cx + m * cy;
      }
      ...
      combined_w += 0.5 * segments[argmin_ip][2];
      combined_w += 0.5 * segments[argmax_ip][2];
