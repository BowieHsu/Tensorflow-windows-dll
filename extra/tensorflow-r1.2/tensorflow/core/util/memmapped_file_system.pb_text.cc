...
#include "tensorflow/core/util/memmapped_file_system.pb_text-impl.h"
...
void AppendProtoDebugString(
    ::tensorflow::strings::ProtoTextOutput* o,
    const ::tensorflow::MemmappedFileSystemDirectory& msg) {
  for (int i = 0; i < msg.element_size(); ++i) {
    o->OpenNestedMessage("element");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.element(i));
    ...
    if (identifier == "element") {
      ...
      do {
        ...
        if (!::tensorflow::internal::ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->add_element())) return false;
